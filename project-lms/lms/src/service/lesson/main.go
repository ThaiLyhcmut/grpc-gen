...
import (
	...
	"github.com/thaily/lms/src/service/lesson/handler"
...
func main() {
	...
	if err := godotenv.Load("./lesson.env"); err != nil {
		log.Printf("Warning: .env file not found: %v", err)
	}

	// Initialize file logger
	if err := logger2.InitFileLogger("lesson-service", "log"); err != nil {
	...
	if err := tls.VerifyCertificatesExist("lesson"); err != nil {
		log.Fatalf("TLS certificate verification failed: %v", err)
	}

	// Load TLS credentials
	creds, err := tls.LoadServerTLSCredentials("lesson")
	if err != nil {
	...
	pb.RegisterLessonServiceServer(grpcServer, h)

	log.Printf("LessonService listening on port %s", port)
