...
import (
	...
	"github.com/thaily/lms/src/service/submission/handler"
...
func main() {
	...
	if err := godotenv.Load("./submission.env"); err != nil {
		log.Printf("Warning: .env file not found: %v", err)
	}

	// Initialize file logger
	if err := logger2.InitFileLogger("submission-service", "log"); err != nil {
	...
	if err := tls.VerifyCertificatesExist("submission"); err != nil {
		log.Fatalf("TLS certificate verification failed: %v", err)
	}

	// Load TLS credentials
	creds, err := tls.LoadServerTLSCredentials("submission")
	if err != nil {
	...
	pb.RegisterSubmissionServiceServer(grpcServer, h)

	log.Printf("SubmissionService listening on port %s", port)
