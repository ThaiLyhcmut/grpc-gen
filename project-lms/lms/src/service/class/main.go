...
import (
	...
	"github.com/thaily/lms/src/service/class/handler"
...
func main() {
	...
	if err := godotenv.Load("./class.env"); err != nil {
		log.Printf("Warning: .env file not found: %v", err)
	}

	// Initialize file logger
	if err := logger2.InitFileLogger("class-service", "log"); err != nil {
	...
	if err := tls.VerifyCertificatesExist("class"); err != nil {
		log.Fatalf("TLS certificate verification failed: %v", err)
	}

	// Load TLS credentials
	creds, err := tls.LoadServerTLSCredentials("class")
	if err != nil {
	...
	pb.RegisterClassServiceServer(grpcServer, h)

	log.Printf("ClassService listening on port %s", port)
